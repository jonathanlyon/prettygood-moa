...
st.set_page_config(
    page_title="PrettyGood Mixture Of Agents Powered by PrettyGood AI AaaP",
    page_icon='static/favicon.ico',
        menu_items={
        'About': "## PrettyGood Mixture-Of-Agents \n Powered by [PrettyGood AI AaaP](https://www.prettygood.agency)"
    },
    layout="wide"
)
valid_model_names = [
...
with st.sidebar:
    ...
    st.title("PrettyGood MOA Configuration")
    with st.form("Agent Configuration", border=False):
        ...
        tooltip = "PrettyGood Agents in the layer agent configuration run in parallel _per cycle_. Each layer agent supports all initialization parameters of [Langchain's ChatGroq](https://api.python.langchain.com/en/latest/chat_models/langchain_groq.chat_models.ChatGroq.html) class as valid dictionary fields."
    ...
    st.markdown("""
    ### Credits
    - PG MOA: [PrettyGood AI AaaP](https://www.prettygood.agency)
    - LLMs: [Groq](https://groq.com/)
    - Paper: [arXiv:2406.04692](https://arxiv.org/abs/2406.04692)
    """)

# Main app layout
st.header("PrettyGood Mixture of Agents", anchor=False)
st.write("A Mixture of Agents experimental architecture by PrettyGood AI AaaP Powered by Groq LLMs.")
st.image("./static/moa_groq.svg", caption="PrettyGood AI AaaP Mixture of Agents Workflow", width=1000)

# Display current configuration
with st.expander("Current PrettyGood MOA Configuration", expanded=False):
